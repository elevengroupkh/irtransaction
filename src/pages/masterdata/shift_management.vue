  ...
  <q-page class="q-pa-sm">
    ...
    <q-dialog v-model="new_customer">
      <q-card style="width: 600px; max-width: 60vw;">
        <q-card-section>
          <div class="text-h6">
            <span v-if="edit==false">Add new Shift</span>
            <span v-if="edit==true">Edit Shift</span>
            ...
            <q-list>
              <q-item>
                <q-item-section>
                  ...
                  <q-input dense outlined v-model="customer.name" label="Shift Name"/>
                </q-item-section>
              </q-item>
              <q-item>
                <q-item-section>
    ...
    export default {
        ...
        methods: {
            exportTable() {
                ...
                const status = exportFile("shift-management.csv", content, "text/csv");
            ...
            async deleteRecord (id) {
              await this.$axios.delete('api/shift/'+this.deleteid)
              .then((response)=>{
                this.getRecord()
              })
            },
            async editRecord() {
              this.loading = true
              await this.$axios.put('api/shift/'+this.editid, this.customer)
              .then((response)=>{
                this.getRecord()
              })
            },
            ...
            async addRecord() {
              await this.$axios.post('api/shift', this.customer)
              .then((response)=>{
                this.getRecord()
